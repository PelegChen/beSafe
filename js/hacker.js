...
G = G || {}
globalObject: {
    G.Q = _Dummy_object.QuestionsArray; // _Dummy_object
    G.css = {};
    ...
    G.sound = {};
    G.mgmt = {};
    G.mgmt.nameOfGame = _Dummy_object.nameOfGame
...
function langSet() {
    ...
    G.TXT = {
        ...
        firstStageBegin: "דוגמא איך להתחיל",
        continueinTheProcces: "המשך בתהליך ",
        findAndgetIp: 'למציאת כתובת הרשת ',
        findWeaknessFirewall: ' ',
        enterWithUser: ' ',
        findThewebSiteScanApps: '',
        BuildAttacvirusAgainst: '',
      ...
      לכל שאלה יש רק תשובה אחת נכונה. אם לא עניתם על התשובה הנכונה, תוכלו לנסות שוב`,
        ...
        youMustPlantVirusEveryAnswer:
            `עליך לפרוץ לפלאפון של ילד.  כדי להתחיל לחץ
     על מקש ימני של העכבר ואז על בדוק.  באזור הלבן
     שיפתח בחר ב Console ועקוב אחרי ההוראות.
    ` + ``,
        summary: ' סיכום',
        following: `להלן `,
        replay: `משחק חוזר`,
...
function clickAnswer(elem) {
    G.divs.explain = Elm('explain', 'img');
    var img = G.divs.explain;
    stl(img, { position: 'fixed', width: "90%", height: "90%" })
    img.src = "data/example.png"
    document.body.appendChild(img)
    setTimeout(() => {
        G.divs.explain.remove()
    }, 6000);

    return
    if (G.mgmt.qNumber === 504) { storeInLocal('confirmReset'); return }
    playSound('Consoletyping', 'loop')
    if (elem === 'rightAnswerClick') {
        elem = {}; let rightAnswer = G.Q[G.mgmt.qNumber][G.mgmt.solutionCol]
    ...
    function wrongAnswerAnimation(num) {
        ...
        function deletAnswer() {
            ...
            if (text.length + 1 < e) {
                G.mgmt.isAnswering = false; IpadGrahpic('wrong'); playSound('Consoletyping', 'pause');
                if (G.mgmt.qNumber < 500) { playSound('wrongAnswer') }; return
            } else { G.divs.ans[num].innerHTML = str; e++; setTimeout(() => { deletAnswer() }, 100) }
        }
        deletAnswer()
    }
    function answerAnimation(text, isWin) {
        ...
        function typeSolution() {
            ...
            if (text.length + 1 <= e) {
                nextQuesion(); playSound('Consoletyping', 'pause');
                if (G.mgmt.qNumber < 500) { playSound('rightAnswer') }
                ; return
            } else { G.css.typeSolution = " " + str; e++; setTimeout(() => { typeSolution() }, ms) }
        }
        typeSolution()
    ...
    function nextQuesion() {
        ...
        function fadeOut() {
            ...
            if (op > 0) { setTimeout(() => { fadeOut() }, time) } else if (!G.mgmt.isFinalAnsInChapter) {
                if (G.mgmt.qNumber === 500) {
                    G.mgmt.qNumber = G.hacks.lastqNumber; plus = 0;
                ...
                setQuestion(G.mgmt.qNumber + plus)
            } else if (G.mgmt.isFinalAnsInChapter) {
 ...
 <input id="clear" type="submit" value="${G.TXT.newGame}" style=" ${formStyle}font-size: 4vmin">
    ...
    function mouseInOut(evnt) {
        ...
        if (evnt.type === 'mouseover') {
            evnt.target.style.backgroundColor = G.linkColor_Hover
        ...
        if (evnt.type === 'mouseout') {
            evnt.target.style.backgroundColor = G.linkColor
...
function ledEvent(e) {
    ...
    function toggleHolo() {
        ...
        function slideIpadUp(y, delta = -2, end = -50, tm = 10) {
            ...
            if (delta < 0 && y < end || delta > 0 && y > end - 3) {
                G.mgmt.ipadMoves = false;

                if (delta < 0) {
                    G.divs.holoContainer.style.opacity = 0; fadeHolo(0.2, -1, 80);
                    G.divs.holoContainer.style.visibility = "visible";
                } else { G.divs.holoContainer.style.visibility = "hidden"; G.divs.holoContainer.style.opacity = 1 };
        ...
        if (G.mgmt.isHolo) {
            ...
            fadeHolo(1, 1);
            slideIpadUp(-50, 2, 3, 14); G.mgmt.isHolo = false
        } // 10
...
function buildBoard() {
    ...
    stl(G.divs.holoScreen, myStyle('text'), {
        "position": "fixed",
        "opacity": '1',
        'overflow': 'hidden',
        "fontSize": '4vmin',
...
function setQuestion(num) {
    ...
    function blinkCursor() {
        ...
        if (G.divs.question.isCursorOn) { G.divs.question.isCursorOn = false } else { G.divs.question.isCursorOn = true };
        let cursor = "▉";
...
`
    ...
    function changeShieldOrScull(rows) {
        ...
        function vanishTxt(t, line0) {
            ...
            let inlarger = 5
            t += (inlarger / 40);
            if (t < 7) { setTimeout(() => { vanishTxt(t, line0) }, 45) } else {
                row.style.opacity = 1; row.animaAtate = false;
                return
            }
        }
        function runText(line, start = 10, len = 5, isSquer = false) {
            ...
            function runSameRow(t) {
                ...
                t++;
                if (t < 50) { setTimeout(() => { runSameRow(t) }, 150 + (line * 3)) } else {
                    Id("scullId" + line).innerHTML = rowText; Id("scullId" + line).animaAtate = false; runText(rndLine(), start, len, isSquer)
                }
 ...
 |_| |_|\\__,_|\\___|_|\\_\\___|_|`
    ...
    function startGame() {
        ...
        fadeOutPromise1(ipadCover, 10).then(() => {
            ...
            fadeOutPromise1(fullBlackScreen, 50).then(() => {
                fullBlackScreen.remove()
    ...
    if (com === 'startGame') { startGame() } else {
        startScreen();
...
function IpadGrahpic(type0) {
    ...
    function getIp() {
        ...
        function FullIpWasfoundAnimation() {
            ...
            for (let n = 1; n <= G.mgmt.numOfsuccess; n++) {
                ...
                let sn = n;
                setTimeout((n) => {
                    MoveC(G.hacks.ipLocations[sn - 1][0], G.hacks.ipLocations[sn - 1][1],
                        G.hacks.ipLocations[sn][0], G.hacks.ipLocations[sn][1], 2, lineObj)
                }, sn * tmAdder);
                saveN = n;
            }
    ...
    function fireWall() {
        ...
        function setFirewallPieces(numOfFWPieces) {
            ...
            for (let i = 1; i < numOfFWPieces; i++) {
                ...
                if (defences.length < 2) {
                    defences = defences.concat(defences_save)
        ...
        function drawFireWallIpad() {
            ...
            if (G.mgmt.numOfsuccess == 0) {
                ipadCover.innerHTML += `<br><br><font style="${textStl}">` + G.TXT.serachIngComponenet + "</font>"; setBG(ipadFireWallBGColor);
            } else {
                ipadCover.style.opacity = '1';
    ...
    function user() {
        ...
        for (let i1 = 1; i1 <= G.mgmt.numOfsuccess; i1++) {
            ...
            if (q === undefined) { } else {
                q.value = q.data
                if (q.id === 'firstName' && G.mgmt.numOfsuccess === i1) {
                    Id('passportIMG').src = Id('passportIMG').data; 'data/passports/passport (19).jpg';
                ...
                if (q.id === 'submitButton') {
                    q.style.backgroundColor = 'rgb(144, 238, 144)'; colorBorderSubmit(q, 60, 1);
